...
# Compile and run commands for Rust code
...
{system_info}
Your response will be written to a file called main.rs and then compiled and executed; the compilation command is:
{compile_command}
...
# Function to transform Python to Rust
...
def port(model, python):
    ...
    # Some models prefix with a bare "rust" line; strip it to avoid rustc errors.
    lines = reply.lstrip().splitlines()
    if lines and lines[0].strip().lower() == "rust":
        reply = "\n".join(lines[1:])
    return reply

# Function to convert Python to Rust and run benchmark
...
# Function to compile and run Rust code

def compile_and_run(cpp_code, work_dir):
    results = []
    timings = []
    try:
        main_rs_path = os.path.join(work_dir, "main.rs")
...
# Function to convert Python to Rust across models in parallel and benchmark
...
with gr.Blocks() as ui:
    gr.Markdown("# 🚀 Python to Rust Code Converter & Benchmark")
    gr.Markdown("Convert Python code to optimized Rust and benchmark the performance")
    
    with gr.Row():
        python = gr.Textbox(label="Python code:", lines=28, value=python_code_to_convert)
        cpp = gr.Textbox(label="Rust code:", lines=28)
