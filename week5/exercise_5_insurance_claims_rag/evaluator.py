...
def get_group_label(test) -> str:
    """Group results by category when available, otherwise by final decision."""
    category = getattr(test, "category", None)
    if category and category != "uncategorized":
        return category
    return getattr(test, "final_decision", "uncategorized")


def run_retrieval_evaluation(progress=gr.Progress()):
    """Run retrieval evaluation and yield updates."""
    progress(0, desc="Starting retrieval evaluation...")
    total_mrr = 0.0
    total_ndcg = 0.0
    total_coverage = 0.0
    ...
    for test, result, prog_value in evaluate_all_retrieval():
        if test is None:
            progress(prog_value, desc="Running retrieval evaluation...")
            continue
        count += 1
        ...
        group_label = get_group_label(test)
        category_mrr[group_label].append(result.mrr)
    ...
    if count == 0:
        ...
        <div style="padding: 20px; text-align: center; color: #999;">
            No retrieval tests found. Add tests to evaluation/tests.jsonl.
        </div>
        """
    ...
    category_data = []
    for group_label, mrr_scores in category_mrr.items():
        avg_group_mrr = sum(mrr_scores) / len(mrr_scores)
        category_data.append({"Group": group_label, "Average MRR": avg_group_mrr})
...
def run_answer_evaluation(progress=gr.Progress()):
    ...
    progress(0, desc="Starting answer evaluation...")
    total_accuracy = 0.0
    ...
    for test, result, prog_value in evaluate_all_answers():
        if test is None:
            progress(prog_value, desc="Running answer evaluation...")
            continue
        count += 1
        ...
        group_label = get_group_label(test)
        category_accuracy[group_label].append(result.accuracy)
    ...
    if count == 0:
        empty_html = """
        <div style="padding: 20px; text-align: center; color: #999;">
            No answer tests found. Add tests to evaluation/tests.jsonl.
        </div>
        """
        return empty_html, pd.DataFrame([])
    ...
    # Create final bar chart data
    category_data = []
    for group_label, accuracy_scores in category_accuracy.items():
        avg_group_accuracy = sum(accuracy_scores) / len(accuracy_scores)
        category_data.append({"Group": group_label, "Average Accuracy": avg_group_accuracy})
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                answer_chart = gr.BarPlot(
                    x="Group",
                    y="Average Accuracy",
                    title="Average Accuracy by Group",
                    y_lim=[1, 5],
                    height=400,
                )
