...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1.""",
    ...
    judge_response = completion(base_url="https://openrouter.ai/api/v1", model=MODEL, messages=judge_messages, response_format=AnswerEval, api_key=os.getenv("OPENROUTER_API_KEY"))
...
def evaluate_all_retrieval(max_workers: int | None = None):
    ...
    max_workers = max_workers or min(50, total_tests)
    completed = 0
...
def evaluate_all_answers(max_workers: int | None = None):
